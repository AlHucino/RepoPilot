...
def get_project_session_dir(cwd: str | Path) -> Path:
    ...
    # 同名项目可能出现在不同目录；把绝对路径 hash 进目录名，避免
    # session 文件互相覆盖，同时路径名仍保留项目名方便人工排查。
...
def save_session_snapshot(
    *,
    ...
    now = time.time()
    # 保存前再次 sanitize，保证中断在 tool_use 之后的会话不会被原样写成
    # provider 不能恢复的坏历史。
    ...
    # latest 支持 `oh --continue`，session-{id} 支持按 ID 恢复。
    # 两份内容相同，换取更简单的读取路径和用户体验。
...
def export_session_markdown(
    *,
    ...
    parts: list[str] = ["# RepoPilot Session Transcript"]
