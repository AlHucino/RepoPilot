...
class McpClientManager:
    ...
    def __init__(self, server_configs: dict[str, object]) -> None:
        ...
        # status 是给 UI/dry-run/diagnostics 展示的连接事实来源。
        # 即使某个 server 失败，也不能阻止整个 OpenHarness runtime 启动。
    ...
    async def connect_all(self) -> None:
        ...
        # MCP server 在启动阶段尽力连接：成功的 server 会贡献 tools/resources，
        # 失败的 server 只记录状态，避免一个外部集成拖垮整个 agent 会话。
    ...
    async def call_tool(self, server_name: str, tool_name: str, arguments: dict[str, Any]) -> str:
        ...
        # 到这里时 MCP tool 已经被包装成普通 BaseTool；manager 只负责
        # 找到对应 session、调用远端工具，并把多形态结果压成文本。
    ...
    async def read_resource(self, server_name: str, uri: str) -> str:
        ...
        # resource 和 tool 分开暴露：resource 更像只读上下文材料，
        # tool 则是可执行动作。UI/模型可以据此做不同风险判断。
        session = self._sessions.get(server_name)
        if session is None:
            status = self._statuses.get(server_name)
            detail = status.detail if status else "unknown server"
