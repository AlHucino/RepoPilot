...
async def build_runtime(
    *,
    ...
    # 插件、MCP 和内置工具都会汇入同一个 ToolRegistry，这样后续 agent loop
    # 不需要区分工具来源，权限、hook、结果回填也能走统一路径。
    ...
    # System prompt 是运行时动态产物：它不仅包含基础身份，还会合并权限模式、
    # skills、项目记忆、issue/pr context 等会影响 agent 行为的上下文。
    ...
    # tool_metadata 是跨工具、跨轮次传递的轻量状态。这里只放可恢复的摘要、
    # 最近读过的文件、compact checkpoint 等，避免把 runtime 对象写进 session。
    restored_metadata = {
        "permission_mode": settings.permission.mode.value,
        "read_file_state": [],
        "invoked_skills": [],
    ...
    # Sandbox 是工具执行的第二道边界：权限层决定“能不能调用工具”，
    # sandbox 决定“工具即使运行了，能访问什么文件和网络”。
...
async def handle_line(
    ...
    *,
    ...
    # Natural-language input follows the normal conversation path: refresh the
    # system prompt for current context and permissions, then submit to engine.
    settings = bundle.current_settings()
    if bundle.enforce_max_turns:
        bundle.engine.set_max_turns(settings.max_turns)
