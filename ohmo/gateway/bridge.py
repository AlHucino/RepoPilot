...
class OhmoGatewayBridge:
    ...
    async def run(self) -> None:
        ...
        while self._running:
            try:
                # Bridge 是 channel bus 和 agent runtime 的边界：channel adapter
                # 只负责收消息，真正的会话路由、打断、执行都在这里发生。
            ...
            # session_key 决定远程消息复用哪一个 RuntimeBundle。群聊会把 sender
            # 纳入 key，避免不同用户共享同一段 agent memory。
            logger.info(
            ...
            # 每个 session 同时只保留一个处理任务；新消息会取消旧任务，
            # 这比并发回复更符合 IM 里的“以最新消息为准”体验。
