...
from openharness.agentops.evals import run_eval_suite
from openharness.agentops.trace import export_run_markdown, list_runs, load_run_events, summarize_run

app = typer.Typer(
    help="RepoTrace AgentOps: trace, audit, and evaluate software-engineering agent runs.",
    no_args_is_help=True,
)
trace_app = typer.Typer(help="Inspect persistent AgentOps traces.", no_args_is_help=True)
eval_app = typer.Typer(help="Run deterministic AgentOps eval suites.", no_args_is_help=True)
...
def trace_export(
    ...
    if fmt == "json":
        ...
        out = Path(output).expanduser().resolve() if output else project / ".repotrace" / "traces" / f"run-{run_id}.json"
