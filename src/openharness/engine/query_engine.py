...
class QueryEngine:
    ...
    async def submit_message(self, prompt: str | ConversationMessage) -> AsyncIterator[StreamEvent]:
        ...
        # 这一层负责“会话状态”，真正的模型-工具循环在 run_query()。
        # 分层后，UI 可以只关心 StreamEvent，而不用直接维护 messages。
        user_message = (
            prompt
        ...
        # Resume 或中断后可能留下不完整的 tool_use/tool_result 尾巴；
        # 在追加新用户消息前清理，避免 provider 拒绝后续请求。
        ...
        trace_recorder = TraceRecorder.start(
            cwd=self._cwd,
            model=self._model,
            request_summary=user_message.text,
            mode="submit",
        )
        self._tool_metadata["trace_recorder"] = trace_recorder
        ...
        # coordinator 模式会额外注入 worker/context 信息，但不直接污染
        # QueryEngine 的持久消息，避免普通 resume 时混入运行时上下文。
        coordinator_context = self._build_coordinator_context_message()
        if coordinator_context is not None:
            query_messages.append(coordinator_context)
        try:
            async for event, usage in run_query(context, query_messages):
                trace_recorder.record_stream_event(event, usage)
            ...
            if self._tool_metadata.get("trace_recorder") is trace_recorder:
                self._tool_metadata.pop("trace_recorder", None)
            # 无论模型成功、工具失败还是用户中断，都尽量更新 session/memory。
            # 这样长会话可以恢复到最近的安全状态。
    ...
    async def continue_pending(self, *, max_turns: int | None = None) -> AsyncIterator[StreamEvent]:
        ...
        trace_recorder = TraceRecorder.start(
            cwd=self._cwd,
            model=self._model,
            request_summary="continue pending tool loop",
            mode="continue",
        )
        self._tool_metadata["trace_recorder"] = trace_recorder
        trace_status = "completed"
