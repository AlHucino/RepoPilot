...
class Settings(BaseModel):
    ...
    def resolve_profile(self, name: str | None = None) -> tuple[str, ProviderProfile]:
        ...
        # profile 是一整套“模型后端配置档案”：provider、协议格式、
        # 认证来源、默认模型、base_url 等都通过它一起切换。
        profile_name = (name or self.active_profile or os.environ.get("OPENHARNESS_PROFILE") or "").strip() or "claude-api"
        if profile_name not in profiles:
            # 兼容旧版扁平配置：如果 active_profile 不存在，就把 top-level
            # provider/model/base_url 临时折叠成一个 fallback profile。
    ...
    def materialize_active_profile(self) -> Settings:
        ...
        # 运行时多数代码仍读取 settings.model/settings.api_format 等扁平字段；
        # materialize 会把当前 profile 投影回这些字段，降低旧代码迁移成本。
    ...
    def resolve_auth(self) -> ResolvedAuth:
        ...
        if auth_source in {"codex_subscription", "claude_subscription"}:
            # subscription profile 复用本地 CLI/OAuth 凭据，不应当走普通 API key。
            # 这也是 README 中 workflow 和 backend preset 分开的原因之一。
        ...
        if auth_source == "copilot_oauth":
            # Copilot client 自己管理 device flow/token，这里只返回一个
            # 统一的 ResolvedAuth，让 runtime 的 provider factory 能继续工作。
            return ResolvedAuth(
                provider="copilot",
        ...
        if profile.credential_slot:
            # credential_slot 允许多个同类 provider profile 各自保存 key，
            # 避免 Kimi/DeepSeek/OpenRouter 等兼容接口共用一个全局凭据。
