...
class OhmoSessionRuntimePool:
    ...
    中文导读：ohmo 并没有重写 agent 引擎，而是按 IM 会话复用
    OpenHarness 的 RuntimeBundle，并替换成 ohmo 自己的 memory/session/prompt。
    """
    ...
    async def get_bundle(
        ...
        if bundle is not None:
            ...
            if bundle_cwd != session_cwd:
                # 同一个聊天会话如果切换了工作目录，就必须重建 runtime；
                # 否则工具 cwd、project memory 和权限边界都会指向旧项目。
                logger.info(
                ...
                # 每条远程消息都可能改变 relevant memory/context，所以复用
                # bundle 时仍要刷新 system prompt。
        ...
        # 这里复用 build_runtime()，但注入 ohmo workspace 下的私有 skills、
        # plugins、memory 和 session backend，使个人助手与项目助手隔离。
