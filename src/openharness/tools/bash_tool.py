...
class BashTool(BaseTool):
    ...
    async def execute(self, arguments: BashToolInput, context: ToolExecutionContext) -> ToolResult:
        ...
        if preflight_error is not None:
            # bash tool 没有人机交互 stdin，脚手架类命令如果卡在选择题上
            # 会让 agent 挂住，所以在启动进程前先给出可操作的失败信息。
            return ToolResult(
        ...
        try:
            process = await create_shell_subprocess(
                ...
                cwd=cwd,
                prefer_pty=True,
                # 非交互执行是设计边界：模型不能在子进程里临时回答 prompt，
                # 需要用户显式传 --yes/--non-interactive 或自己先完成初始化。
        ...
        try:
            ...
            # 超时时尽量保留 partial output，方便模型/用户判断是测试失败、
            # 安装卡住还是服务进程没有退出。
