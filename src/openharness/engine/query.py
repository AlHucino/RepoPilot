...
async def run_query(
    ...
    while context.max_turns is None or turn_count < context.max_turns:
        ...
        # 在每轮模型调用前先压缩上下文。这样模型永远看到“当前可承载”的
        # messages，而不是等 provider 报 context too long 后才被动失败。
        ...
        # 非多模态模型不能直接接收 ImageBlock；这里通过 vision fallback
        # 先把图片转文字，让同一条用户消息仍能被普通文本模型处理。
        ...
        try:
            ...
            if not reactive_compact_attempted and _is_prompt_too_long_error(exc):
                ...
                # reactive compact 是兜底路径：auto compact 没预测到超长时，
                # 根据 provider 的真实错误强制压缩一次并重试当前轮。
        ...
        if len(tool_calls) == 1:
            # 单工具顺序执行可以立即把开始/结束事件流给 UI，用户能看到进度。
            tc = tool_calls[0]
            ...
            # 多工具并发执行提升吞吐，但结果仍按 tool_use 顺序回填，
            # 保证下一轮 provider 能稳定匹配每个 tool_result。
        ...
        # 工具结果在协议上是一条 user message。模型下一轮会基于这些
        # tool_result 决定继续调用工具，还是生成最终回答。
...
async def _execute_tool_call(
    ...
    recorder = _trace_recorder(context)
    if context.hook_executor is not None:
        # PRE_TOOL_USE 是工具执行前的最后一道可扩展拦截点，可用于审计、
        # 企业策略、危险命令阻断或外部审批。
    ...
    # 权限检查必须使用归一化后的路径/命令，否则模型可以用相对路径、
    # root 参数或不同字段名绕过 path rule。
    ...
    if not decision.allowed:
        if decision.requires_confirmation and context.permission_prompt is not None:
            # default mode 下的写操作不直接失败，而是把决策交给 UI/用户。
            # 用户拒绝后仍返回 tool_result，让模型知道边界在哪里。
            ...
            if recorder is not None:
                recorder.record_permission_confirmation(
                    tool_name=tool_name,
                    confirmed=confirmed,
                    reason=decision.reason,
                )
...
def _trace_recorder(context: QueryContext) -> TraceRecorder | None:
    if not isinstance(context.tool_metadata, dict):
        return None
    candidate = context.tool_metadata.get("trace_recorder")
    if isinstance(candidate, TraceRecorder):
        return candidate
    return None
